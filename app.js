...
//retorna apenas pokemons válidos
...
//retorna os IDs dos pokemons
const getPokemonsIds = pokeApiResults => pokeApiResults.map(({ url}) => {
  const urlAsArray = url.split('/')
  return urlAsArray.at(urlAsArray.length - 2)
})

//retorna as imagens dos pokemons, baseados nos IDs
const getPokemonsImg = async ids => {
...
const getPokemons = async () => {
  try {
    ...
    const imgs = await getPokemonsImg(ids)
    const pokemons = ids.map((id, i) => ({id:id, name: pokeApiResults[i].name, types: types[i], imgUrl: imgs[i] }))
    return pokemons
  } 
catch (error) {
